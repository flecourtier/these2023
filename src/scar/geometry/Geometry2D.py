...
class Circle(ParametricCurves):
    ...
    def c(self,t): # t \in [0,1]
        ...
        x = self.x0 + self.r*torch.cos(t)
        y = self.y0 + self.r*torch.sin(t)
    ...
    def c_prime(self,t):
        ...
        x = -2.*PI*self.r*torch.sin(t)
        y = 2.*PI*self.r*torch.cos(t)
...
class Astroid(ParametricCurves):
    ...
    def c_prime(self,t):
        ...
        x = -6*PI*torch.cos(t)**2*torch.sin(t)
        y = 6*PI*torch.cos(t)*torch.sin(t)**2
...
class SmoothAstroid(ParametricCurves):
    ...
    def c(self,t): # t \in [0,1]
        ...
        x = self.r * torch.cos(t) + 0.5*torch.cos(t)**3
        y = self.r * torch.sin(t) + 0.5*torch.sin(t)**3
    ...
    def c_prime(self,t):
        ...
        x = -2*PI*self.r*torch.sin(t) - 3*PI*torch.sin(t)*torch.cos(t)**2
        y = 2*PI*self.r*torch.cos(t) + 3*PI*torch.cos(t)*torch.sin(t)**2
...
class SmoothCardioid(ParametricCurves):
    ...
    def c(self,t):
        ...
        x = self.r * ( 2*torch.cos(t) - 0.7*torch.cos(2*t) )
        y = self.r * ( 2*torch.sin(t) - 0.7*torch.sin(2*t) )
    ...
    def c_prime(self, t):
        ...
        x = self.r * ( -4*PI*torch.sin(t) + 2.8*PI*torch.sin(2*t) )
        y = self.r * ( 4*PI*torch.cos(t) - 2.8*PI*torch.cos(2*t) )
...
class Pumpkin(ParametricCurves):
    ...
    def c(self,t):
        ...
        x = torch.cos(t)+0.3*torch.cos(3*t)+0.1*torch.cos(5*t)
        y = torch.sin(t)+0.3*torch.sin(3*t)+0.1*torch.sin(5*t)
    ...
    def c_prime(self, t):
        ...
        x = -2*PI*torch.sin(t) - 1.8*PI*torch.sin(3*t) - PI*torch.sin(5*t)
        y = 2*PI*torch.cos(t) + 1.8*PI*torch.cos(3*t) + PI*torch.cos(5*t)
...
class Bean(ParametricCurves):
    ...
    def c(self,t):
        ...
        x = (torch.sin(t)**self.a+torch.cos(t)**self.b)*torch.cos(t)
        y = (torch.sin(t)**self.a+torch.cos(t)**self.b)*torch.sin(t)
    ...
    def c_prime(self, t):
        ...
        x = (2*PI*self.a*torch.sin(t)**(self.a-1)*torch.cos(t) - 2*PI*self.b*torch.sin(t)*torch.cos(t)**(self.b-1))*torch.cos(t) - 2*PI*(torch.sin(t)**self.a + torch.cos(t)**self.b)*torch.sin(t)

        y = (2*PI*self.a*torch.sin(t)**(self.a-1)*torch.cos(t) - 2*PI*self.b*torch.sin(t)*torch.cos(t)**(self.b-1))*torch.sin(t) + 2*PI*(torch.sin(t)**self.a + torch.cos(t)**self.b)*torch.cos(t)
