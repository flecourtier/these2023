    ...
    \begin{center}
        ...
        \begin{minipage}{0.9\linewidth}
            \begin{algorithm}[H]
                ...
                \For{\( j \ge 0 \)}{
                    Solve the linear system \( F(\vec{U}_k^{(j)}) + F'(\vec{U}_k^{(j)}) \delta_k^{(j+1)} = 0 \) for \( \delta_k^{(j+1)} \)\;
                    Update \( \vec{U}_k^{(j+1)} = \vec{U}_k^{(j)} + \delta_k^{(j+1)} \)\;
                }
            \end{algorithm}
        \end{minipage}
    \end{center}
...
\begin{frame}{3 types of initialization}
	\begin{itemize}
		...
		with $\bm{x}^{(i)}=\big(x^{(i)},y^{(i)}\big)$ the $i$-th dofs coordinates of $[V_h^0]^2 \times Q_h \times W_h$ and $U_\theta$ the PINN.}
