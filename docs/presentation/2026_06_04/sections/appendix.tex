...
\begin{appendixframe}{Heated cavity test case}
	...
	\textbf{Stationary incompressible Navier-Stokes equations (with buoyancy and gravity)\only<1>{\footnote[frame,1]{\textcolor{darkred}{The approach can be extended to other test cases.}}} :}
...
\begin{appendixframe}{Weak form - Additive approach}
    \textbf{Weak problem\footnote[frame,1]{Problem solved using Newton's algorithm with the correction vector initialized to $0$.} :} Find $C_h^+=(\textcolor{Cyan}{\bm{C}_{h,\bm{u}}^+}, \textcolor{blue}{C_{h,p}^+}, \textcolor{ForestGreen}{C_{h,T}^+}) \in M_h^{\, 0}$ s.t., \; $\forall (\bm{v}_h, q_h, w_h) \in M_h^{\, 0}$,
...
\begin{appendixframe}{Numerical costs}	
    ...
    % \vspace{-5pt}
    $N_\text{dofs}$ and execution time required to reach the same global $L^2$ relative error $e$ :\footnote[frame,1]{Error defined as the sum of the $L^2$ relatives errors on $\bm{u}$ and $T$.}
    ...
    \begin{minipage}{0.66\linewidth}
        \begin{tikzpicture}
            \node[anchor=south west, inner sep=0] (img) at (0,0) {\includegraphics[height=2.4cm]{images/appendix/costs_param1.pdf}};
