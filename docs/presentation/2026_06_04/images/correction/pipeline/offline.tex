...
\begin{document}
    \begin{tcolorbox}[
        ...
        \centering
        \begin{tikzpicture}
            \draw[draw=black,dashed] (0,0) rectangle ++ (5.1,2.5);	
            \node at (2.5,1.85) {\Large \textbf{Inputs}};
            \node[anchor=west] at (0.1,1.1) {\large $\bm{x}\in\Omega$ : space coordinates};
            \node[anchor=west] at (0.1,0.5) {\large $\bm{\mu}\in\mathcal{M}$ : parameter};
            
            \draw[->, black, line width=2pt] (5.5,1.25) -- (6.5,1.25);
            
            \node[draw=none, inner sep=0pt] at (8.5,1.55) {\includegraphics[width=3cm]{network.png}};
            ...
            \node at (14.5,1.1) {\LARGE$\textcolor{red}{u_\theta}$\large$(\bm{x},\bm{\mu})$ : prediction of};
            \node at (14.5,0.5) {\large the PDE solution $u(\bm{x},\bm{\mu})$};
        \end{tikzpicture}
    \end{tcolorbox}
\end{document}