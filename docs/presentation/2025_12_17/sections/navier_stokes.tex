...
\begin{frame}{Newton method}
    ...
    with $F:\mathbb{R}^{N_h} \to \mathbb{R}^{N_h}$ the non-linear operator associated to \eqref{eq:weakform_nl} and $\vec{U}\in \mathbb{R}^{N_h}$ the unknown vector. %\quad\refappendix{frame:basis}
...
\begin{frame}{Newton method - Additive approach}
    \vspace{-5pt}
    Denoting $N_h$ the dimension of $M_h^+$, we want to solve the non-linear system: %\hfill \tiny $N_h$ : number of degrees of freedom.
